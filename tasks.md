...
- [x] 修正 `auth.ts` 类型定义 (LoginResp, UserMeResp)
- [x] 优化 `LoginPage.vue` 登录逻辑 (调用 `/auth/me` 获取角色)
- [x] 实现 `StudentHome.vue` 咨询师列表展示 (调用 `/api/v1/consultants`)

## 下一步计划
- [ ] 启动后端并验证 Swagger 文档 (`http://localhost:8080/swagger-ui/index.html`)
- [ ] 启动前端并测试登录流程
- [ ] 测试咨询师列表展示
